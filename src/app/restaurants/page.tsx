    ...
    <main>
        ...
        <div className="relative mx-auto max-w-[1200px] px-5">
          ...
          <SectionReveal delay={0.2}>
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-[#CBD5E1]">
              AI-powered intelligence built for multi-unit restaurant operators. Track <span className="highlight-gold">prime cost across every location</span>, benchmark performance, capture missed calls, and get coached on what to fix — all before your first store visit.
            </p>
          </SectionReveal>
              ...
              <SectionReveal key={i} delay={i * 0.1}>
                <div className="content-card rounded-2xl p-8 md:p-10">
                  <div className="flex items-start gap-4">
                    <point.icon size={24} className="mt-1 shrink-0 text-[#E88B5C]" />
                    <div>
                      <h3 className="text-xl font-bold text-[#2D3748]">{point.title}</h3>
                      <p className="mt-3 text-sm leading-relaxed text-[#6B7280]">{point.problem}</p>
                      <div className="mt-4 rounded-lg border border-accent-blue/20 bg-accent-blue/5 p-4">
                        <p className="flex items-start gap-2 text-sm leading-relaxed text-[#4A5568]">
          ...
          <SectionReveal delay={0.1}>
            <div className="content-card mt-10 rounded-2xl p-8 md:p-10">
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                {kpis.map((kpi) => (
                  <div key={kpi.label} className="rounded-xl border border-[#E5E7EB] bg-[#FAFAFA] p-5 text-center">
                    <kpi.icon size={24} className="mx-auto text-accent-blue" />
                    <p className="mt-3 text-sm font-bold text-[#2D3748]">{kpi.label}</p>
                  </div>
                ))}
              </div>
              <div className="mt-8">
                <ChartPlaceholderSvg className="w-full" />
              </div>
            </div>
          </SectionReveal>
        </div>
        ...
        <div className="mx-auto max-w-[1200px] px-5">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {archetypes.map((arc, i) => (
              <SectionReveal key={i} delay={i * 0.1}>
                <div className="content-card flex h-full flex-col rounded-2xl p-8">
                  <div className="mb-4 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-[#2D3748]">{arc.name}</p>
                      <p className="text-xs text-[#9CA3AF]">{arc.role} · {arc.locations}</p>
                    </div>
                  </div>
                  <p className="flex-1 text-sm leading-relaxed text-[#6B7280]">{arc.description}</p>
      ...
      <section className="py-16 md:py-20">
        <div className="mx-auto max-w-[1200px] px-5 text-center">
          <SectionReveal>
            <h2 className="font-heading text-3xl font-medium md:text-4xl">Built by restaurant operators, for restaurant operators</h2>
